...
foreach($datatoshow as $rows){
        ...
        $textfordisplay=$textfordisplay.((key($rows)!=$keyoflast)?"":(isset($delprocess)?"<td><a href='".$delprocess.addslashes($rows[$txnidname])."' OnClick=\"sendTokentoDelete()\"'>Del</a></td>":"").(isset($editprocess)?"<td><a href='".$editprocess.addslashes($rows[$txnidname])."'>Edit</a></td></tr>":""));
