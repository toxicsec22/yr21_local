...
if (isset($itemcode)){
    ...
    include_once '../generalinfo/unionlists/BECSList.php'; 
        ...
        $sql='SELECT a.*,s.BECSName as `Supplier/Client/Branch`,if(Defective=1,"D",if(Defective=2,"DC","G")) as DefectiveOrGood, MONTH(Date) AS `Month` FROM ItemAct a LEFT JOIN `BECSList` s on a.BECSNo=s.BECSNo AND a.BECS=s.BECS WHERE a.Date IS NOT NULL '.$defectcondi.' ORDER BY a.Date;';
    ...
    if(isset($_GET['NotSessionBranch'])){
      $title='Branch No. '.$_GET['BranchNo'].': '.comboBoxValue($link,'1branches','BranchNo',$_GET['BranchNo'],'Branch');
		} else {
			 $title='Branch No. '.$_SESSION['bnum'].': '.$_SESSION['@brn'];
		}
