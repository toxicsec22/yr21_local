...
case 'PerDept':
  ...
  JOIN (SELECT JobLevelID, jc.JobClassNo, FORMAT(MinRate,0) AS `MINIMUM`, FORMAT(MinRate*(1+PercentMintoMed/100),-1) AS MEDIAN, FORMAT(MinRate*(1+PercentMintoMed/100)*(1+PercentMedtoMax/100),-1) AS MAXIMUM  FROM attend_0joblevels jl JOIN attend_0jobclass jc ON jc.JobClassNo=jl.JobClassNo GROUP BY jl.JobLevelID ORDER BY JobClassNo,JobLevelID) jl ON jl.JobLevelID=p.JobLevelID
