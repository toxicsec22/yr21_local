...
$cnts++;
	...
	$sql0='CREATE TEMPORARY TABLE tempPoints AS SELECT '.$totalp.' AS `Points`,FormulaName,`A1` AS Education,`A2` AS Experience,`A3` AS `Technical Skills / Functional Skills`,`B4` AS `Analysis & Problem Solving`,`B5` AS `Nature of External Relations`,`B6` AS `Nature of Internal Relations`,`B7` AS `Complexity of Work`,`B8` AS `Physical Effort /Working Conditions`,`B9` AS `Planning and Controlling`,`B10` AS `Company Assets`,`B11` AS `Confidentiality`,`B12` AS `Leading and Responsibility`,`B13` AS `Contribution to Organization`, jrp.PositionID AS TxnID,jrp.PositionID,Position,jrp.EncodedByNo,jrp.Posted FROM hr_1jobratingplantemp1 jrp JOIN attend_0positions p ON jrp.PositionID=p.PositionID JOIN hr_1jobratingweights jrw ON jrp.WJRID=jrw.WJRID';
	$stmt0=$link->prepare($sql0); $stmt0->execute();
