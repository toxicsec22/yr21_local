...
echo '<tr>';
	...
	echo '<td style="padding:10px;width:25%;" valign="top">';
		...
		$sqlm='SELECT cp.FullName AS Supervisor,MobileNo FROM hr_2perfevalmain pem JOIN attend_30currentpositions cp ON pem.SupervisorIDNo=cp.IDNo JOIN 1_gamit.0idinfo id ON pem.SupervisorIDNo=id.IDNo JOIN attend_30currentpositions cp2 ON pem.IDNo=cp2.IDNo WHERE SelfCompleted=1 AND SuperCompleted=0 AND LENGTH(MobileNo)=11 '.$condition.' GROUP BY SupervisorIDNo';
		$stmtno=$link->query($sqlm); $resno=$stmtno->fetchAll(); $numbers='';
		foreach ($resno as $rno){
			$numbers.=$rno['MobileNo'].';';
		}
	...
	echo '<td style="padding:10px;width:25%;" valign="top">';
		...
		$sqlm='SELECT cp.FullName AS DeptHead,MobileNo FROM hr_2perfevalmain pem JOIN attend_30currentpositions cp ON pem.DeptHeadIDNo=cp.IDNo JOIN 1_gamit.0idinfo id ON pem.DeptHeadIDNo=id.IDNo JOIN attend_30currentpositions cp2 ON pem.IDNo=cp2.IDNo WHERE SelfCompleted=1 AND SuperCompleted=1 AND DeptHeadConfirm=0 AND LENGTH(MobileNo)=11 '.$condition.' GROUP BY DeptHeadIDNo';
		$stmtno=$link->query($sqlm); $resno=$stmtno->fetchAll(); $numbers='';
		foreach ($resno as $rno){
			$numbers.=$rno['MobileNo'].';';
		}
