...
SELECT 
    ...
    UNION ALL SELECT 
        `DateIN` AS `DateIN`,
        "Transfer-In" AS `From`,
        `TransferNo` AS `TransferNo`, "B" AS `BECS`, 
        `BranchNo` AS `BranchNo`,
        `ItemCode` AS `ItemCode`,
    ...
    UNION ALL SELECT 
        `Date` AS `Date`,
        "Sale" AS `From`,
        `SaleNo` AS `SaleNo`, IF(ClientNo<9999,"E","C") AS `BECS`, 
    ...
    UNION ALL 
        SELECT `Date`,"Add-on" AS `From`, `SaleNo`, IF(ClientNo<9999,"E","C") AS `BECS`, 
        `ClientNo`, `ItemCode`, (`ao`.`Qty` * -(1)) AS `Qty`, 
    \'\' as `UnitCost`, \'\' as `UnitPrice`, \'\' as `SerialNo`, `BranchNo`, 
    case when Approved=1 then "Approved Add-on" when Approved=0 then "For approval Add-on" when Approved=2 then "Rejected Add-on" end as `Remarks`, `txntype`, \'\' as `Defective`
FROM (`invty_2salesubaddons` `ao` JOIN `invty_2sale` `s` ON ((`s`.`TxnID` = `ao`.`TxnID`)))
WHERE MONTH(`s`.`Date`)<='.$asofmonth.'
