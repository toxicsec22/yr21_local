...
foreach($datatoshow as $rows){
	...
	$total=(isset($coltototal)?$total+$rows[$coltototal]:0);  
		...
		//showeditprocess
					 .((isset($editprocess) AND 1==(isset($rows['showeditprocess'])?$rows['showeditprocess']:'1'))?'<td><a href="'.$editprocess.$rows[$txnid].(isset($addlfield)?'&'.$addlfield.'='.$rows[$addlfield]:'').'" '.((isset($opennewtab) AND $opennewtab)?' target=_blank':'').'>'.$editprocesslabel.'</a></td>':(isset($rows['showeditprocess'])?'<td></td>':''))
					  .(isset($editprocess2)?'<td><a href="'.$editprocess2.$rows[$txnid].(isset($addlfield)?'&'.$addlfield.'='.$rows[$addlfield]:'').'">'.$editprocesslabel2.'</a></td>':'')
					 .((isset($addlprocess) AND 1==(isset($rows['showaddlprocess'])?$rows['showaddlprocess']:'1'))?'<td><a href='.$addlprocess.$rows[$txnid].'&action_token='.$_SESSION['action_token'].'>'.$addlprocesslabel.'</a></td>':(isset($rows['showaddlprocess'])?'<td></td>':'')).(isset($addlprocess2)?'<td><a href='.$addlprocess2.$rows[$txnid].'&action_token='.$_SESSION['action_token'].'>'.$addlprocesslabel2.'</a></td>':'')
