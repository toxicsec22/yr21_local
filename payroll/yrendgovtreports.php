...
tr:nth-child(even) {background-color: #f2f2f2;}
	...
	$sql2='Select *, Concat(ii.SurName,\' | \',ii.FirstName,\' | \',ii.MiddleName) as `LastName FirstName MiddleName`,ii.SurName as LastName,CompanyName,Branch,ii.TIN AS TIN,CONCAT(ii.DateHired,\' | \', ifnull(ii.DateResigned,"")) as `EmploymentFrom EmploymentTo`,BasicDaily,BasicMonthly,format((Basic+OT+DeM-AbsenceBasic-UndertimeBasic+'.(($resultc['Resigned']==1)?'ifnull(13thBasicCalc,0)':'ifnull(`13th`,0)').'),2) as `GROSS COMPENSATION INCOME`,'.(($resultc['Resigned']==1)?'ifnull(format(13thBasicCalc,2),0) as `13th`':'ifnull(`13th`,0) as `13th`').',format(DeM,2) as `DeMinimisRate`,format((yg.`SSS-EE`+yg.`PhilHealth-EE`+yg.`PagIbig-EE`),2) as `TOTAL SSS, PHIC & PAGIBIG`,format(((Basic+OT+DeM-AbsenceBasic-UndertimeBasic+'.(($resultc['Resigned']==1)?'ifnull(13thBasicCalc,0)':'ifnull(`13th`,0)').')-'.(($resultc['Resigned']==1)?'ifnull(13thBasicCalc,0)':'ifnull(`13th`,0)').'-DeM-(yg.`SSS-EE`+yg.`PhilHealth-EE`+yg.`PagIbig-EE`)),2) as `SALARIES & OTHER FORMS OF COMPENSATION`,format(TaxWithheld,2) from yrgross yg left join 1employees e on e.IDNo=yg.IDNo left join 1_gamit.0idinfo ii on ii.IDNo=yg.IDNo left join 1companies c on c.CompanyNo=e.RCompanyNo left join attend_30currentpositions cp on cp.IDNo=yg.IDNo '.(($resultc['Resigned']==1)?'left join payroll_21dailyandmonthlyofresigned dam on dam.IDNo=yg.IDNo left join payroll_26yrtotaland13thmonthcalc ytt on ytt.IDNo=yg.IDNo ':' left join payroll_21dailyandmonthly dam on dam.IDNo=yg.IDNo ').'';
