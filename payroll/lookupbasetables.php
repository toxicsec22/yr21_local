    ...
    $whichqry=!isset($_GET['w'])?'':$_GET['w'];
     switch ($whichqry){
	    ...
	    $title='Salary Structure'; 
            ...
            $sql='SELECT jl.*, JobClassification, FORMAT(MinRate,0) AS `MINIMUM`, FORMAT(MinRate*(1+PercentMintoMed/100),-1) AS MEDIAN, FORMAT(MinRate*(1+PercentMintoMed/100)*(1+PercentMedtoMax/100),-1) AS MAXIMUM  FROM attend_1joblevel jl JOIN attend_0jobclass jc ON jc.JobClassNo=jl.JobClassNo GROUP BY jl.JobLevelNo ORDER BY JobClassNo,JobLevelNo;';
            
            
            $columnnames=array('JobClassification','JobLevelNo','MINIMUM','MEDIAN','MAXIMUM');
            ...
            $formdesc='';
            $sql='SELECT jc.*, 
(SELECT GROUP_CONCAT(DISTINCT Position ORDER BY Position SEPARATOR "<BR>") FROM attend_1joblevel jl LEFT JOIN attend_0positions p ON jl.JobLevelNo=p.JobLevelNo WHERE RIGHT(jl.JobLevelNo,1)=1 AND jl.JobClassNo=jc.JobClassNo GROUP BY jl.JobLevelNo)  AS `Level 1`,
(SELECT GROUP_CONCAT(DISTINCT Position ORDER BY Position SEPARATOR "<BR>") FROM attend_1joblevel jl LEFT JOIN attend_0positions p ON jl.JobLevelNo=p.JobLevelNo WHERE RIGHT(jl.JobLevelNo,1)=2 AND jl.JobClassNo=jc.JobClassNo GROUP BY jl.JobLevelNo) AS `Level 2`,
(SELECT GROUP_CONCAT(DISTINCT Position ORDER BY Position SEPARATOR "<BR>") FROM attend_1joblevel jl LEFT JOIN attend_0positions p ON jl.JobLevelNo=p.JobLevelNo WHERE RIGHT(jl.JobLevelNo,1)=3 AND jl.JobClassNo=jc.JobClassNo GROUP BY jl.JobLevelNo) AS `Level 3`
 FROM attend_0jobclass jc ORDER BY jc.JobClassNo;';
            
            $hidecount=TRUE;
            $columnnames=array('JobClassification','Level 1','Level 2','Level 3');
            echo '<div style="margin-left: 15%;">';
            include('../backendphp/layout/displayastableonlynoheaders.php'); 
            echo '</div>';
            ?>
...
$sql='SELECT *, FORMAT(MinRate,0) AS MinimumRateinPeso FROM attend_1joblevel ORDER BY JobClassNo,JobLevelNo;';
// echo $sql; exit();
$txnidname='JobLevelNo';
$columnnames=array('JobLevelNo','MinimumRateinPeso','PercentMintoMed','PercentMedtoMax'); 
$width='20%';
$title=''; $formdesc='';
