...
<?php
	...
	IF(nopaID<>4,'.$sqlchkmain.' ReqStatus=1 AND Served=1 AND ApprovedByEO=1),'.$sqlchkmain2.') AS checker3
	 FROM hr_nopaholdingtable ht LEFT JOIN attend_1positions p1 ON ht.PositionIDFrom=p1.PositionID LEFT JOIN attend_1positions p2 ON ht.PositionIDTo=p2.PositionID LEFT JOIN 1branches b1 ON ht.BranchNoFrom=b1.BranchNo LEFT JOIN 1branches b2 ON ht.BranchNoTo=b2.BranchNo WHERE IDNo=';
	...
	<?php
		...
		if(isset($_GET['set'])){
			...
			foreach($res AS $row){
				...
				if($fromwhatnopa==1 OR $fromwhatnopa==5){
					$sqlpos = 'SELECT PositionID, Position FROM attend_1positions ORDER BY Position';
