...
// check if allowed
$allowed=array(671,672,673,674,675,6714);$allow=0;
...
//,'RDateHired'		
    ...
    break;
	 ...
	 case 8:
	 unset($editprocesslabel,$editprocess);
        if (allowedToOpen(674,'1rtc')){
		echo comboBox($link,'SELECT FullName, IDNo FROM `attend_30currentpositions` cp WHERE PositionID=36 ORDER BY FullName;','IDNo','FullName','namelist');
		
        ?><br><br>
        
        <form method='post' action='employeeinfo.php?calledfrom=81'>
        Change Assignment:  &nbsp &nbsp Date of Change<input type='date' size="6" name='DateofChange' value='<?php echo date('Y-m-d');?>'>
          &nbsp &nbsp Branch <input type='text' size="8" name='Branch' list='branchnames' autocomplete=off>
		  &nbsp &nbsp IDNo <input type='text' size="10" name='FullName' list='namelist' autocomplete=off>
          &nbsp &nbsp Remarks <input type='text' name='Remarks' autocomplete=off>
        ...
        <?php
		 include_once('../generalinfo/lists.inc');
        renderlist('branchnames');
       // TABLE attend_1stlfixedbranch HAS BEEN DELETED
        $title='STL Fixed Branch';
		$formdesc='</i><br/><b><a href=employeeinfo.php?calledfrom=9>STL Fixed Branch Assignment History</a></b><i>';
        $columnnames=array('DateofChange','BranchNo','Branch','IDNo','STL');
        $columnsub=$columnnames;
					$sql='SELECT sfb.DateofChange, b.BranchNo, Branch, concat(e.Nickname," ",e.Surname) as STL, sfb.IDNo'
                    . ' FROM attend_1stlfixedbranch sfb JOIN (SELECT MAX(DateofChange) AS MaxDate, BranchNo FROM attend_1stlfixedbranch GROUP BY BranchNo) sfl ON sfb.BranchNo=sfl.BranchNo AND sfb.DateofChange=sfl.MaxDate LEFT join `1employees` e on sfb.IDNo=e.IDNo '
                    . ' join `1branches` b on b.BranchNo=sfb.BranchNo '
                    . ' where b.Active=1';
				$width='70%';	
        include('../backendphp/layout/displayastable.php'); 
        } else { echo 'No permission'; exit;}   
	...
	if (allowedToOpen(6741,'1rtc')) {
		...
		$sql='INSERT INTO attend_1stlfixedbranch SET DateofChange="'.$_POST['DateofChange'].'",BranchNo='.$branchno.',IDNo='.$idno.',Remarks="'.$_POST['Remarks'].'",EncodedByNo='.$_SESSION['(ak0)'].',TimeStamp=NOW();';
		
		// echo $sql; exit();
		$stmt=$link->prepare($sql); $stmt->execute();
	}	
	header ("Location:employeeinfo.php?calledfrom=8");
	...
	case 9://team leader assignment fixed history
        unset($editprocess); 
        ...
        $txnid='TxnID'; 
		 ...
		 echo comboBox($link,'SELECT sfb.BranchNo,Branch FROM attend_1stlfixedbranch sfb LEFT JOIN 1branches b ON sfb.BranchNo=b.BranchNo WHERE DateofChange=CURDATE() AND sfb.EncodedByNo='.$_SESSION['(ak0)'].' GROUP BY BranchNo','BranchNo','Branch','branchlist');
		
	 $formdesc=  '</i><br>Delete IDNo (Current Date Encode Only)';
	 $formdesc.= "<form action='employeeinfo.php?calledfrom=62' method='POST' enctype='multipart/form-data' style='display: inline'>
   ...
   $formdesc.='<br><b><a href=employeeinfo.php?calledfrom=8>Change Fixed Assignment</a></b><i>';
   
            $sql='select DateofChange,sfb.BranchNo,Branch,sfb.IDNo,concat(e.Nickname," ",e.Surname) AS FullName,concat(e2.Nickname," ",e2.Surname) AS EncodedBy,sfb.Remarks,sfb.EncodedByNo,sfb.TimeStamp FROM attend_1stlfixedbranch sfb LEFT JOIN 1branches b ON sfb.BranchNo=b.BranchNo LEFT JOIN 1employees e ON sfb.IDNo=e.IDNo LEFT JOIN 1employees e2 ON sfb.EncodedByNo=e2.IDNo ORDER BY `TimeStamp` DESC;';
        include('../backendphp/layout/displayastable.php');
   ...
   $formdesc.='<br><b><a href=employeeinfo.php?calledfrom=7>Change Assignment</a></b><i>';
		...
		if (allowedToOpen(6741,'1rtc')) {
			 ...
			 $branchno=companyandbranchValue($link,'1branches','Branch',addslashes($_POST['Branch']),'BranchNo');
			$sql='DELETE FROM `attend_1stlfixedbranch` WHERE `IDNo`='.$_POST['IDNo'].' AND BranchNo='.$branchno.' AND DateofChange=CURDATE() AND EncodedByNo='.$_SESSION['(ak0)'].';';
