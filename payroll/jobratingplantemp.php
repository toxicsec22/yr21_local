...
if (in_array($which,array('LookupPoints','LookupJobGrade','PositionList'))){
	...
	if($which=='LookupJobGrade'){
		$sqlp='SELECT jrp.WJRID,FormulaName,Posted,PostedTS,PostedByNo FROM hr_1jobratingplantemp1 jrp JOIN hr_1jobratingweights jrw ON jrp.WJRID=jrw.WJRID WHERE PositionID='.intval($_GET['PositionID']);
...
switch ($which)
{
	...
	END)
		...
		as Degree FROM formulatable ft JOIN hr_1jobratingplantemp1 jrp ON ft.WJRID=jrp.WJRID WHERE ft.cnt='.$cntr.' AND PositionID='.$positionid.' UNION ALL ';
		$cntr++;
	}
...
$cnts++;
	...
	$sql0='CREATE TEMPORARY TABLE tempPoints AS SELECT '.$totalp.' AS `Points`,FormulaName,`A1` AS Education,`A2` AS Experience,`A3` AS `Technical Skills / Functional Skills`,`B4` AS `Analysis & Problem Solving`,`B5` AS `Nature of External Relations`,`B6` AS `Nature of Internal Relations`,`B7` AS `Complexity of Work`,`B8` AS `Physical Effort /Working Conditions`,`B9` AS `Planning and Controlling`,`B10` AS `Company Assets`,`B11` AS `Confidentiality`,`B12` AS `Leading and Responsibility`,`B13` AS `Contribution to Organization`, jrp.PositionID AS TxnID,jrp.PositionID,Position,jrp.EncodedByNo,jrp.Posted FROM hr_1jobratingplantemp1 jrp JOIN attend_1positions p ON jrp.PositionID=p.PositionID JOIN hr_1jobratingweights jrw ON jrp.WJRID=jrw.WJRID';
	$stmt0=$link->prepare($sql0); $stmt0->execute();
	...
	if(isset($_GET['PositionID'])){
		...
		$sqlf='SELECT * FROM hr_1jobratingplantemp1 WHERE PositionID='.intval($_GET['PositionID']);
		$stmtf=$link->query($sqlf); $rowf=$stmtf->fetch();
	...
	if(isset($_GET['edit'])){
		...
		$sql = 'UPDATE hr_1jobratingplantemp1 SET '.$insupd.' WHERE '.((allowedToOpen(59053,'1rtc'))?'':'EncodedByNo='.$_SESSION['(ak0)'].' AND ').' Posted=0 AND PositionID='.$_GET['edit'];
	} else {
		$sql = 'INSERT INTO hr_1jobratingplantemp1 SET '.$insupd.'';
	}
	...
	if (!allowedToOpen(59052,'1rtc')) { echo 'No permission'; exit; }
	require_once $path.'/acrossyrs/logincodes/confirmtoken.php';
	$sql = 'DELETE FROM hr_1jobratingplantemp1 WHERE '.((allowedToOpen(59053,'1rtc'))?'':'EncodedByNo='.$_SESSION['(ak0)'].' AND ').' Posted=0 AND PositionID='.$_GET['PositionID'];
	$stmt=$link->prepare($sql); $stmt->execute();
	...
	if (!allowedToOpen(59052,'1rtc')) { echo 'No permission'; exit; }
	require_once $path.'/acrossyrs/logincodes/confirmtoken.php';
	$sql = 'UPDATE hr_1jobratingplantemp1 SET Posted=IF(Posted=1,0,1),PostedByNo='.$_SESSION['(ak0)'].',PostedTS=NOW() WHERE '.((allowedToOpen(59053,'1rtc'))?'':'EncodedByNo='.$_SESSION['(ak0)'].' AND ').' PositionID='.$_GET['PositionID'];
	$stmt=$link->prepare($sql); $stmt->execute();
