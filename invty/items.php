...
if (in_array($which,array('List','EditSpecifics'))){
   ...
   $sql='SELECT i.*,IF(Auto=1,"Y","") AS `Auto?`,IF(Ref=1,"Y","") AS `Ref?`,IF(Aircon=1,"Y","") AS `Aircon?`, Category, e.Nickname as EncodedBy, i.ItemCode AS TxnID, MovementType,
	 IF(RequiredSerialNo<>0,"Yes","No") AS Required_Serial_No, IF(WithBarcode<>0,"Yes","No") AS With_Barcode FROM invty_1items i
...
switch ($which){
   ...
   ?>
	 ...
	 ';
	 $itemcode='select ItemCode from invty_1items  where ItemCode not IN (5257,8134) AND ItemCode<30000 order by  ItemCode Desc  limit 1';
