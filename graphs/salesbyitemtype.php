...
$which=isset($_GET['w'])?$_GET['w']:'SalesByItemType';
...
if (in_array($which,array('SalesByItemType','SalesByItemTypeYr'))){
...
$sqlm='SELECT (SELECT IFNULL(TRUNCATE((SUM(s.Qty*s.UnitPrice)/1000000),2),0) FROM `invty_2sale` m JOIN `invty_2salesub` s ON m.`TxnID`=s.`TxnID` JOIN invty_1items i ON s.ItemCode=i.ItemCode WHERE txntype IN (1,2,5,10) AND (m.ClientNo NOT BETWEEN 1000 AND 9999) AND (m.ClientNo NOT BETWEEN 15001 AND 15005) '.(($which=='SalesByItemType')?'AND Month(`Date`)='.$txndate.'':'').' ';
...
case 'SalesByItemType':
case 'SalesByItemTypeYr':

$title='Sales By Item Type';
echo '<title>'.$title.'</title>';
echo '<form action="salesbyitemtype.php?w=SalesByItemType" method="POST">Month: <input type="text" size="5" value="'.$txndate.'" name="monthno"> <input type="submit" name="btnSubmit" value="Lookup"> <a href="salesbyitemtype.php?w=SalesByItemTypeYr">Sales By Item Type (All Months)</a></form> <br>';

if($which=='SalesByItemType'){
...
$reportid='1';
	...
	$sqlc=$sqlm.') AS TotSales';
	$stmttot=$link->query($sqlc); $restot=$stmttot->fetch();
	
	$alltot=$restot['TotSales'];
	
   $sqlcsales=$sqlm.' AND Auto=0 AND Ref=0 AND Aircon=0) AS TotSales,"Unknown" AS ItemType';   
   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=1 AND Ref=0 AND Aircon=0) AS TotSales,"Auto Only" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=0 AND Ref=1 AND Aircon=0) AS TotSales,"Ref Only" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=0 AND Ref=0 AND Aircon=1) AS TotSales,"Aircon Only" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=1 AND Ref=1 AND Aircon=0) AS TotSales,"Auto and Ref" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=1 AND Ref=0 AND Aircon=1) AS TotSales,"Auto and Aircon" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=0 AND Ref=1 AND Aircon=1) AS TotSales,"Ref and Aircon" AS ItemType';   
   $sqlcsales.=' UNION '. $sqlm.'  AND Auto=1 AND Ref=1 AND Aircon=1) AS TotSales,"Auto,Ref and Aircon" AS ItemType';   
   ...
   $stmtcsales=$link->query($sqlcsales); $res=$stmtcsales->fetchAll();
	...
	$sqlcmain='INSERT INTO graphreport11 SET Label="'.$label.'", ReportID=1,ReportTitle="Sales Per Item Type"';
