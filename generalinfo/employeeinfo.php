...
//,'RDateHired'
    ...
    case 3: // edit this year's employees
	 ...
	 case 7://edit team leader assigns
        ...
        $columnnames=array('DateofChange','BranchNo','Branch','InventoryPlanner','TLFullName','SAMName','FieldSpecialist','BranchCoordinator','BranchSupport','OpsManager','CreditAnalyst');//'AssignedAR',
        $columnsub=$columnnames;
					$sql='SELECT bg.DateofChange, b.BranchNo, Branch, concat(e.Nickname," ",e.Surname) as TLFullName, concat(e3.Nickname," ",e3.Surname) as CreditAnalyst, SAM, concat(e4.Nickname," ",e4.Surname) as SAMName, concat(e5.Nickname," ",e5.Surname) AS FieldSpecialist, concat(e9.Nickname," ",e9.Surname) AS BranchCoordinator,concat(e10.Nickname," ",e10.Surname) AS BranchSupport,concat(e6.Nickname," ",e6.Surname) AS OpsManager,concat(e7.Nickname," ",e7.Surname) AS InventoryPlanner FROM attend_1branchgroups bg LEFT join `1employees` e on bg.TeamLeader=e.IDNo  LEFT join `1employees` e3 on bg.CNC=e3.IDNo LEFT join `1employees` e4 on bg.SAM=e4.IDNo  left join `1employees` e5 on bg.FieldSpecialist=e5.IDNo left join `1employees` e9 on bg.BranchCoordinator=e9.IDNo left join `1employees` e10 on bg.BranchSupport=e10.IDNo left join `1employees` e6 on bg.OpsManager=e6.IDNo left join `1employees` e7 on bg.InventoryPlanner=e7.IDNo join `1branches` b on b.BranchNo=bg.BranchNo  where b.Active=1;';
