...
// PURCHASES
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT Date, concat("Supp.Inv# ",SupplierInv), "S" AS `BECS`, p.SupplierNo, CONCAT(SupplierName, " TIN#",s.TIN, " ", s.Address), concat("Inv.Date ",DateofInv), DebitAccountID,BranchNo,FromBudgetOf,  Amount, IFNULL(Forex,1) AS Forex, Amount*IFNULL(Forex,1) AS PHPAmount, "DR", "Purchase", p.TxnID FROM `acctg_2purchasemain` p join `acctg_2purchasesub` ps on `p`.TxnID=`ps`.TxnID JOIN `1suppliers` s on s.SupplierNo=p.SupplierNo  WHERE YEAR(Date)='.$thisyr.' AND MONTH(Date) in ('.$montharray.') and DebitAccountID in '.$acctid.'
UNION ALL 
SELECT Date, concat("Supp.Inv# ",SupplierInv), "S" AS `BECS`, p.SupplierNo, SupplierName, concat("Inv.Date ",DateofInv), CreditAccountID, BranchNo,FromBudgetOf,  Amount*-1, IFNULL(Forex,1) AS Forex, Amount*-1*IFNULL(Forex,1) AS PHPAmount, "CR", "Purchase", p.TxnID  FROM `acctg_2purchasemain` p join `acctg_2purchasesub` ps on `p`.TxnID=`ps`.TxnID JOIN `1suppliers` s on s.SupplierNo=p.SupplierNo  WHERE YEAR(Date)='.$thisyr.' AND MONTH(Date) in ('.$montharray.')  and CreditAccountID in '.$acctid;
...
if(isnull(vchs.TIN),"", concat(" TIN#",vchs.TIN, " ", t.CompanyName, " ", t.Address))), vchm.CreditAccountID, BranchNo,FromBudgetOf, vchs.Amount*-1, IFNULL(Forex,1) AS Forex, Amount*-1*IFNULL(Forex,1) AS PHPAmount, "CR", "CV", vchm.CVNo FROM acctg_2cvmain vchm INNER JOIN acctg_2cvsub vchs ON vchm.CVNo=vchs.CVNo LEFT JOIN `gen_info_1tinforexpenses` t on t.TIN=vchs.TIN WHERE YEAR(Date)='.$thisyr.' AND MONTH(Date) in ('.$montharray.')  and CreditAccountID in '.$acctid;

// SALES 
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT sm.Date, "Sale" AS Expr1, IF(ss.ClientNo<9999,"E","C") AS `BECS`, ss.ClientNo, ifnull(`ClientName`,"") as ClientName, ss.Particulars, ss.DebitAccountID, sm.BranchNo, sm.BranchNo as FromBudgetOf, ss.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr2, "Sale", sm.TxnID
...
UNION ALL
SELECT sm.Date, "Sale" AS Expr1, IF(ss.ClientNo<9999,"E","C") AS `BECS`, ss.ClientNo, ifnull(`ClientName`,"") as ClientName, ss.Particulars, ss.CreditAccountID, sm.BranchNo,sm.BranchNo as FromBudgetOf, Amount*-1 AS Expr2, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Sale", sm.TxnID
FROM acctg_2salemain sm INNER JOIN (acctg_2salesub ss left JOIN `acctg_01uniclientsalespersonfordep` c ON ss.ClientNo = c.ClientNo) ON (sm.TxnID = ss.TxnID)
WHERE Month(sm.Date) in ('.$montharray.')  and CreditAccountID in '.$acctid;


// COLLECTIONS
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT orm.Date, concat("Collect",orm.CollectNo) AS Expr1, IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ifnull(`ClientName`,"") as ClientName, concat(orm.CollectNo," Inv",ForChargeInvNo,"/",`Type`,"/",ifnull(`CheckNo`,"")) AS Expr2, orm.DebitAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, ors.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr3, "Collect", orm.TxnID
...
UNION ALL
SELECT orm.Date, concat("Collect",orm.CollectNo) AS Expr1, IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ifnull(`ClientName`,"") as ClientName, DeductDetails, orm.DebitAccountID as CreditAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, ors.Amount*-1,  1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Collect", orm.TxnID
...
UNION ALL SELECT orm.Date, concat("Collect",orm.CollectNo) AS Expr1, IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ifnull(`ClientName`,"") as ClientName, concat(orm.CollectNo," Inv",ForChargeInvNo,"/",`Type`,"/",ifnull(`CheckNo`,"")) AS Expr2, ors.CreditAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, ors.Amount*-1 AS Expr3,  1 AS Forex, Amount*-1 AS PHPAmount,"CR" AS Expr4, "Collect", orm.TxnID
FROM (acctg_2collectmain orm left JOIN `acctg_01uniclientsalespersonfordep` c ON orm.ClientNo = c.ClientNo) INNER JOIN acctg_2collectsub ors ON  (orm.TxnID = ors.TxnID)
WHERE Month(orm.Date) in ('.$montharray.')  and CreditAccountID in '.$acctid.'
UNION ALL SELECT orm.Date, concat("Collect",orm.CollectNo) AS Expr1, IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ifnull(`ClientName`,"") as ClientName, DeductDetails, ors.DebitAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, ors.Amount AS Expr3,  1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr4, "Collect", orm.TxnID
FROM (acctg_2collectmain orm left JOIN `acctg_01uniclientsalespersonfordep` c ON orm.ClientNo = c.ClientNo) INNER JOIN acctg_2collectsubdeduct ors ON  (orm.TxnID = ors.TxnID)
WHERE Month(orm.Date) in ('.$montharray.')  and ors.DebitAccountID in '.$acctid;

// DEPOSITS and ENCASHMENTS
$sqlalltxns=$sqlalltxns.' UNION ALL
SELECT depm.Date, concat("Dep# ",depm.DepositNo) AS Expr1, IF(deps.ClientNo<9999,"E","C") AS `BECS`, deps.ClientNo as `SuppNo/ClientNo`, concat(ifnull(`ClientName`,"")," ",ifnull(`DepDetails`," ")) AS Expr2, concat(deps.`ForChargeInvNo`, "/",deps.`Type`,"/",ifnull(`CheckNo`,"")) AS Expr3, depm.DebitAccountID, deps.BranchNo,deps.BranchNo as FromBudgetOf, deps.Amount, IFNULL(Forex,1) AS Forex, Amount*IFNULL(Forex,1) AS PHPAmount, "DR","Deposit", depm.TxnID 
...
UNION ALL
SELECT depm.Date, concat("Encash# ",depm.DepositNo) AS Expr1, "B" AS `BECS`, 0, CONCAT(depe.EncashDetails,if(isnull(depe.TIN),"", concat(" ", t.CompanyName))), CONCAT(if(isnull(ApprovalNo),"",concat("approval ",`ApprovalNo`)),if(isnull(depe.TIN),"", concat(" TIN#",depe.TIN, " ", t.Address)))  AS Expr2, depm.DebitAccountID AS CreditAcctID, depe.BranchNo,depe.FromBudgetOf, `Amount`*-1 AS Expr3,  1 AS Forex, Amount*-1 AS PHPAmount,"CR" AS Expr4, "Deposit", depm.TxnID 
FROM acctg_2depositmain depm INNER JOIN acctg_2depencashsub depe ON depm.TxnID = depe.TxnID LEFT JOIN `gen_info_1tinforexpenses` t on t.TIN=depe.TIN 
...
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT bs.DateBounced, CONCAT("Bounced#CR",bm.CRNo,"_",bm.PDCNo), IF(bm.ClientNo<9999,"E","C") AS `BECS`, bm.ClientNo, c.ClientName, PDCNo, 200 AS DebitAccountID, BranchNo,BranchNo as FromBudgetOf, AmountofPDC, 1 AS Forex, AmountofPDC*1 AS PHPAmount,"DR" AS Expr4, "Bounced", bm.UndepPDCId
...
UNION ALL SELECT bs.DateBounced, CONCAT("Bounced#CR",bm.CRNo,"_",bm.PDCNo), IF(bm.ClientNo<9999,"E","C") AS `BECS`, bm.ClientNo, c.ClientName, PDCNo, IFNULL(CreditAccountID,100), BranchNo,BranchNo as FromBudgetOf, `AmountofPDC`*-1 AS Expr3, 1 AS Forex, AmountofPDC*-1 AS PHPAmount,"CR" AS Expr4, "Bounced", bm.UndepPDCId
FROM (`acctg_01uniclientsalespersonfordep` c right JOIN `acctg_3undepositedpdcfromlastperiod` bm ON c.ClientNo = bm.ClientNo) INNER JOIN `acctg_3undepositedpdcfromlastperiodbounced` bs ON  (bm.UndepPDCId = bs.UndepPDCId)
...
$sqlalltxns=$sqlalltxns.' UNION ALL
SELECT tm.Date, "TxfrOUT" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo,ts.ClientBranchNo, ts.Particulars, ts.DebitAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr2, "Interbranch", tm.TxnID
...
UNION ALL
SELECT tm.Date, "TxfrOUT" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo,ts.ClientBranchNo, ts.Particulars, tm.CreditAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount,"CR" AS Expr2, "Interbranch", tm.TxnID
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
if (in_array(300,$acctidarray)){ //Inventory account
        ...
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT ts.DateIN, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 300 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DateIN) in ('.$montharray.')  And ((ts.DateIN Is Not Null) AND (MONTH(ts.DateIN)=MONTH(tm.Date)) AND (YEAR(ts.DateIN)=YEAR(tm.Date))) and DebitAccountID=204';        
        ...
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT tm.Date, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 300 AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
if (in_array(330,$acctidarray)){ //In Transit - Incoming account
        ...
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT tm.Date, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 330 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(tm.Date) in ('.$montharray.')  And (ts.DateIN Is Null) AND Year(tm.Date)='.$thisyr.' and DebitAccountID=204';
        ...
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT tm.Date, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 330 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(tm.Date) in ('.$montharray.')  And (ts.DateIN Is Not Null) AND ((MONTH(ts.DateIN)<>MONTH(tm.Date)) OR (YEAR(ts.DateIN)<>YEAR(tm.Date))) and DebitAccountID=204 
UNION ALL SELECT ts.DateIN, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 330 AS CRID, ts.ClientBranchNo, ts.ClientBranchNo as FromBudgetOf,ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount,"CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DateIN) in ('.$montharray.')  And (ts.DateIN Is Not Null) AND ((MONTH(ts.DateIN)<>MONTH(tm.Date)) OR (YEAR(ts.DateIN)<>YEAR(tm.Date))) and DebitAccountID=204';
}
if (in_array(337,$acctidarray)){ //In Transit - Outgoing account
        // DateIN is NULL (DR ARTradeTxfr CR InvtInTransitOUT)
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT tm.Date, "TxfrOUT" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo, ts.ClientBranchNo, ts.Particulars, 330 AS DRID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(tm.Date) in ('.$montharray.')  And (ts.DateIN Is Null) AND Year(tm.Date)='.$thisyr.' and DebitAccountID=204';
...
UNION ALL SELECT ts.DateIN, "TxfrIN" AS Expr1, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 330 AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DateIN) in ('.$montharray.')  And (ts.DateIN Is Not Null) AND ((MONTH(ts.DateIN)<>MONTH(tm.Date)) OR (YEAR(ts.DateIN)<>YEAR(tm.Date))) and DebitAccountID=204';
}
if (in_array(404,$acctidarray)){ //APTradeTxfr account
        // Month of IN = Month of OUT (DR Inventory CR APTradeTxfr)
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT ts.DateIN, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 404 AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DateIN) in ('.$montharray.')  And ((ts.DateIN Is Not Null) AND (MONTH(ts.DateIN)=MONTH(tm.Date)) AND (YEAR(ts.DateIN)=YEAR(tm.Date))) and DebitAccountID=204';
        // IF DATEIN IS NULL or different months/years as DateOut (DR InTransit CR APTradeTxfr)
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT tm.Date, "TxfrIN" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 404 AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
$sqlalltxns=$sqlalltxns.' UNION ALL
SELECT ts.DatePaid, "TxfrOUTRecvPayment" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo,ts.ClientBranchNo, ts.Particulars, ts.PaidViaAcctID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr2, "Interbranch", tm.TxnID
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DatePaid) in ('.$montharray.')  And (ts.DatePaid Is Not Null) and DebitAccountID=204 and PaidViaAcctID in '.$acctid.'
UNION ALL
SELECT ts.DatePaid, "TxfrOUTRecvPayment" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo,ts.ClientBranchNo, ts.Particulars, ts.DebitAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr2, "Interbranch", tm.TxnID
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT ts.DatePaid, "TxfrINPayment" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, ts.PaidViaAcctID AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount*-1, 1 AS Forex, Amount*-1 AS PHPAmount, "CR" AS Expr3, "Interbranch", tm.TxnID
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
WHERE Month(ts.DatePaid) in ('.$montharray.')  And (ts.DatePaid Is Not Null) and DebitAccountID=204 and PaidViaAcctID in '.$acctid;

if (in_array(404,$acctidarray)){
        $sqlalltxns=$sqlalltxns.' UNION ALL SELECT ts.DatePaid, "TxfrINPayment" AS Expr1, "B" AS `BECS`, tm.FromBranchNo, tm.FromBranchNo, ts.Particulars, 404 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, ts.Amount, 1 AS Forex, Amount*1 AS PHPAmount, "DR" AS Expr3, "Interbranch", tm.TxnID FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
