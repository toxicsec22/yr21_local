...
foreach($datatoshow as $rows){
	...
	$total=(isset($coltototal)?$total+$rows[$coltototal]:0);
        $textfordisplay=$textfordisplay.((key($rows)!=$keyoflast)?"":(isset($formprocess)?'<td><form action='.$formprocess.'>'.$formprocessfields.'<input type=hidden name='.$txnid.' value='.$rows[$txnid].'><input type=submit name=submit value="'.$submitlabel.'"></form></td>':'').(isset($delprocess)?"<td><a href='".$delprocess.addslashes($rows[$txnid])."&action_token=".$_SESSION['action_token']."' OnClick=\" return confirm('Really delete this?');\"'>Del</a></td>":"").(isset($editprocess)?"<td><a href='".$editprocess.addslashes($rows[$txnid])."'>".$editprocesslabel."</a></td>":"").(isset($autoprocess)?"<td><a href='".$autoprocess.addslashes($rows[$txnid])."&action_token=".$_SESSION['action_token']."'>".$autoprocesslabel."</a></td>":"").(isset($autoprocess2)?"<td><a href='".$autoprocess2.addslashes($rows[$txnid])."&action_token=".$_SESSION['action_token']."'>".$autoprocesslabel2."</a></td>":"")."</tr>");
} //end foreach
