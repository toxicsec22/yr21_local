...
foreach($datatoshow as $rows){
	...
	$total=(isset($coltototal)?$total+$rows[$coltototal]:0);  
		...
		//showeditprocess
					 .((isset($editprocess) AND 1==(isset($rows['showeditprocess'])?$rows['showeditprocess']:'1'))?'<td><a href="'.$editprocess.$rows[$txnid].(isset($addlfield)?'&'.$addlfield.'='.$rows[$addlfield]:'').'">'.$editprocesslabel.'</a></td>':(isset($rows['showeditprocess'])?'<td></td>':''))
