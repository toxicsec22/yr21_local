...
$deptcondition='(((IF('.$_SESSION['&pos'].' IN (99,100),supervisorpositionid,IF( '.$_SESSION['&pos'].' IN (SELECT deptheadpositionid FROM `1departments`), deptheadpositionid, supervisorpositionid)))='.$_SESSION['&pos'].' OR p.LatestSupervisorIDNo='.$_SESSION['(ak0)'].') '.((allowedToOpen(618,'1rtc'))?' OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE BranchCoordinator='.$_SESSION['(ak0)'].' OR OpsManager='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE SAM='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE TeamLeader='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE CNC='.$_SESSION['(ak0)'].'))':'').')';
