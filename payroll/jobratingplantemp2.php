...
if (in_array($which,array('LookupPoints','LookupJobGrade','PositionList'))){
	...
	if($which=='LookupJobGrade'){
		$sqlp='SELECT jrp.WJRID,FormulaName,Posted,PostedTS,PostedByNo FROM hr_1jobratingplan jrp JOIN hr_1jobratingweightstemp1 jrw ON jrp.WJRID=jrw.WJRID WHERE PositionID='.intval($_GET['PositionID']);
	...
	FROM hr_1jobratingweightstemp1';
...
switch ($which)
{
	...
	if(isset($_GET['edit'])){
		...
		$sqlf='SELECT * FROM hr_1jobratingweightstemp1 WHERE WJRID='.intval($_GET['WJRID']);
	...
	$stmt=$link->query($sql); $rows=$stmt->fetchAll();
	echo '<title>'.$title.'</title>';
	echo '<h3><font color="green"><u>'.comboBoxValue($link,'hr_1jobratingweightstemp1','WJRID',$_GET['WJRID'],'FormulaName').'</u></font> '.$title.'</h3>';
...
$cnts++;
	...
	$sqlformula='SELECT WJRID,FormulaName FROM hr_1jobratingweightstemp1 ORDER BY FormulaName';
	...
	if(isset($_GET['edit'])){
		...
		$sql = 'UPDATE hr_1jobratingweightstemp1 SET '.$insupd.' WHERE '.((allowedToOpen(59053,'1rtc'))?'':'EncodedByNo='.$_SESSION['(ak0)'].' AND ').' WJRID='.$_GET['WJRID'];
	} else {
		$sql = 'INSERT INTO hr_1jobratingweightstemp1 SET '.$insupd.'';
	}
	// echo $sql; exit();
	$stmt=$link->prepare($sql); $stmt->execute();
	...
	if (!allowedToOpen(59051,'1rtc')) { echo 'No permission'; exit; }
	require_once $path.'/acrossyrs/logincodes/confirmtoken.php';
	$sql = 'DELETE FROM hr_1jobratingweightstemp1 WHERE '.((allowedToOpen(59053,'1rtc'))?'':'EncodedByNo='.$_SESSION['(ak0)'].' AND ').' WJRID='.$_GET['WJRID'];
	$stmt=$link->prepare($sql); $stmt->execute();
