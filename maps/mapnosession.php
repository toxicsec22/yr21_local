...
function add_location(){
	...
	$sql='INSERT INTO maps_tempdata set lat='.$lat.',lng='.$lng.',description="'.$description.'",EncodedByNo=0,TimeStamp=NOW()';
...
from 1branches WHERE lat IS NOT NULL AND lng IS NOT NULL UNION select 0,lat,lng,description,location_status AS isconfirmed from maps_tempdata WHERE lat IS NOT NULL AND lng IS NOT NULL '; 
...
$outside_icon='http://maps.google.com/mapfiles/ms/icons/red-dot.png';


$spanstyle='background-color:white;border:1px solid black;padding:12px;font-size:9pt;';
echo '<b>Legend: </b><br>';
...
echo ' <span style="'.$spanstyle.'">Existing Client: <img src="'.$existingclient_icon.'"></span>';
echo ' <span style="'.$spanstyle.'">Potential Branch: <img src="'.$potentialbranch_icon.'"></span>';
echo ' <span style="'.$spanstyle.'">Potential Client: ('.$caraircon.') <img src="'.$potentialclient_icon.'"></span>';
echo ' <span style="'.$spanstyle.'">Potential Client: ('.$refrigeration.') <img src="'.$potentialclient_icon2.'"></span><br><br>';
echo ' <span style="'.$spanstyle.'">Competitor: <img src="'.$competitor_icon.'"></span>';
echo ' <span style="'.$spanstyle.'">Temporary: <img src="'.$outside_icon.'"></span><br><br>';
...
<script type="text/javascript" 
    ...
    <script>
        ...
        var outside_icon = <?php echo "'".$outside_icon."'";?>  ; //-1
		...
		const locationButton = document.createElement("button");
		 ...
		 const branchmap = {
		<?php 
			$sqlr='select Branch,lat,lng,"#FFFFFF" AS color,radiusKM AS radius from 1branches WHERE lat is not null and lng is not NULL';
					...
					foreach($rowrs AS $rowr){
						echo str_replace($search,$replace,str_replace(' ','_',$rowr['Branch'])).': { center: { lat: '.$rowr['lat'].', lng: '.$rowr['lng'].' }, color: "'.$rowr['color'].'", radius: '.$rowr['radius'].' }, ';
					}
		?> 
        ...
        for (i = 0; i < locations.length; i++) {
            marker = new google.maps.Marker({
                ...
                map: map,
                icon :   (((locations[i][4] === 1 ?  existingbranch_icon  : (locations[i][4] === 2 ?  existingclient_icon  : (locations[i][4] === 3 ?  potentialbranch_icon : (locations[i][4] === 4 ?  potentialclient_icon : (locations[i][4] === 5 ?  competitor_icon : (locations[i][4] === -1 ?  outside_icon : (locations[i][4] === 6 ?  existingclient_icon2 : (locations[i][4] === 7 ?  potentialclient_icon2 : pending_icon)))))))))),
        ...
        function saveData(lat,lng) {
			...
			var url = 'mapnosession.php?add_location&description=' + description + '&lat=' + lat + '&lng=' + lng;
            ...
            downloadUrl(url, function(data, responseCode) {
                ...
                if (responseCode === 200  && data.length > 1) {
					...
					manual_marker.setIcon(outside_icon);
					
                    infowindow.close();
					
					 infowindow.setContent("<div style=' color: red; font-size: 20px;'> Added!! </div>");
