...
case 'SendToVoucher':
   ...
   if(!isset($_REQUEST['MonthNo'])) { exit();}
    ...
    if($_SESSION['(ak0)']==1002){ echo $sql.'<br>';}
    $stmt=$link->prepare($sql); $stmt->execute();
    header('Location:../acctg/addeditsupplyside.php?w=CV&CVNo='.$txnid);
    break;
...
echo '<title>STL Incentives</title>'; // TABLE attend_1stlfixedbranch HAS BEEN DELETED
$sql0='CREATE TEMPORARY TABLE STLFixedBranch AS SELECT sfb.BranchNo,IDNo from attend_1stlfixedbranch sfb JOIN (SELECT MAX(DateofChange) AS MaxDate, BranchNo FROM attend_1stlfixedbranch GROUP BY BranchNo) sfl ON sfb.BranchNo=sfl.BranchNo AND sfb.DateofChange=sfl.MaxDate';
$stmt0=$link->prepare($sql0); $stmt0->execute();


$sql='SELECT FullName,GROUP_CONCAT(b.Branch) AS HandledBranch FROM STLFixedBranch sfb JOIN attend_30currentpositions cp ON sfb.IDNo=cp.IDNo JOIN 1branches b ON sfb.BranchNo=b.BranchNo GROUP BY sfb.IDNo';
	$stmt=$link->query($sql); $result=$stmt->fetchAll();
	echo '<br><h3>STL Handled Branches</h3>';
	foreach ($result AS $res){
		echo $res['FullName'].' (<i>'.$res['HandledBranch']. '</i>)<br>';
	}


// if(date('Y')==$currentyr AND date('m')<=3){
	
echo '<br><form action="#" method="POST">Quarter: <select name="Quarter"><option value=1>1st Quarter</option><option value=2>2nd Quarter</option><option value=3>3rd Quarter</option><option value=4>4th Quarter</option></select> <input type="submit" value="Lookup Branch Target Reach"> <input type="submit" name="btnLookupSTLIncentives" value="Lookup STL Incentives"> <input type="submit" value="Calculate STL Incentives" name="btnCalcSTL"></form>';

/* 
// print_r($_POST); exit();
if(!isset($_POST['Quarter']) AND !isset($_POST['btnCalcSTL'])){
	goto notable;
}


$columnnames=array('BranchNo','Branch','Target'.$_POST['Quarter'].'QReached','Fixed_Incentive'.$_POST['Quarter'].'Q','Addl_Incentive'.$_POST['Quarter'].'Q','Total_Incentive'.$_POST['Quarter'].'Q','STL');
...
//ClassID: 2 - Growth 3 - Prime 4 - Mature
	...
	if($quarter==1){
		$monthnoin='1,2,3';
		$summonthno='SUM(`01`+`02`+`03`)';
		$classcomparison='ClassID AS ClassIDLast,';
		$qtitle=' (1st Quarter)';
	} else if ($quarter==2){
		$monthnoin='4,5,6';
		$summonthno='SUM(`04`+`05`+`06`)';
		$classcomparison='Class1Q AS ClassIDLast,';
		$qtitle=' (2nd Quarter)';
	} else if ($quarter==3) {
		$monthnoin='7,8,9';
		$summonthno='SUM(`07`+`08`+`09`)';
		$classcomparison='Class2Q AS ClassIDLast,';
		$qtitle=' (3rd Quarter)';
	} else {
		$monthnoin='10,11,12';
		$summonthno='SUM(`10`+`11`+`12`)';
		$classcomparison='Class3Q AS ClassIDLast,';
		$qtitle=' (4th Quarter)';
	}
	

	$title='STL Incentives'.$qtitle;
	
	$sql0='CREATE TEMPORARY TABLE CalcIncentiveTable AS SELECT b.BranchNo,'.$classcomparison.'
	
	IFNULL((SELECT TRUNCATE(SUM(Net),2) FROM acctg_6targetscores WHERE BranchNo=b.BranchNo AND MonthNo IN ('.$monthnoin.')),0) AS Sale'.$quarter.'QValue,
	(SELECT FORMAT(Sale'.$quarter.'QValue,0)) AS Sale'.$quarter.'Q,
	IFNULL((SELECT TRUNCATE('.$summonthno.',2) FROM acctg_1yearsalestargets WHERE BranchNo=b.BranchNo),0) AS Target'.$quarter.'QValue,
	(SELECT FORMAT(Target'.$quarter.'QValue,0)) AS Target'.$quarter.'Q,
	(SELECT TRUNCATE((Sale'.$quarter.'QValue/3),2)) AS MonthlyAve'.$quarter.'QValue,
	(SELECT FORMAT(MonthlyAve'.$quarter.'QValue,0)) AS MonthlyAve'.$quarter.'Q,
	IF((SELECT Sale'.$quarter.'QValue)>(SELECT Target'.$quarter.'QValue),"Yes","") AS Target'.$quarter.'QReached,
	IF((SELECT Target'.$quarter.'QReached)="Yes",
	
	(CASE 
	WHEN ClassID=1 THEN "2000"
	WHEN ClassID=2 THEN "3000"
	WHEN ClassID=3 THEN "4000"
	WHEN ClassID=4 THEN "10000"
	END)
	
	,"") AS Fixed_Incentive'.$quarter.'Q,
	
	(CASE 
	WHEN (SELECT MonthlyAve'.$quarter.'QValue)<(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+1).') THEN 1 
	WHEN (SELECT MonthlyAve'.$quarter.'QValue)>=(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+1).') AND (SELECT MonthlyAve'.$quarter.'QValue)<(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+2).') THEN 2 
	WHEN (SELECT MonthlyAve'.$quarter.'QValue)>=(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+2).') AND (SELECT MonthlyAve'.$quarter.'QValue)<(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+3).') THEN 3 
	WHEN (SELECT MonthlyAve'.$quarter.'QValue)>=(SELECT CutOffMin FROM 0branchclass WHERE ClassID='.($quarter+3).') THEN 4 END) AS ClassID'.$quarter.'Q, Branch,
	(CASE 
		WHEN ClassID=1 AND (SELECT ClassID'.$quarter.'Q)>ClassID AND (SELECT ClassID'.$quarter.'Q)-ClassID=1 THEN "500"
		WHEN ClassID=2 AND (SELECT ClassID'.$quarter.'Q)>ClassID AND (SELECT ClassID'.$quarter.'Q)-ClassID=1 THEN "1200"
		WHEN ClassID=3 AND (SELECT ClassID'.$quarter.'Q)>ClassID AND (SELECT ClassID'.$quarter.'Q)-ClassID=1 THEN "4000"
		WHEN ClassID=1 AND (SELECT ClassID'.$quarter.'Q)>ClassID AND (SELECT ClassID'.$quarter.'Q)-ClassID=2 THEN "5000"
	ELSE "0"
	END) AS Addl_Incentive'.$quarter.'Q,(SELECT Fixed_Incentive'.$quarter.'Q) + (SELECT Addl_Incentive'.$quarter.'Q) AS Total_Incentive'.$quarter.'Q,CONCAT(e.Nickname," ",e.Surname) AS STL
	
	FROM 1branches b JOIN acctg_6targetscores ts ON b.BranchNo=ts.BranchNo JOIN 0branchclass bc ON b.ClassLastYr=bc.ClassID JOIN STLFixedBranch sfb ON ts.BranchNo=sfb.BranchNo LEFT JOIN 1employees e ON sfb.IDNo=e.IDNo WHERE CURDATE()>=(Anniversary + INTERVAL 4 MONTH + INTERVAL 1 DAY) AND PERIOD_DIFF(DATE_FORMAT(CURDATE(), "%Y%m"), DATE_FORMAT(Anniversary, "%Y%m") )>=4 GROUP BY BranchNo ORDER BY STL ASC,Branch;';
	$stmt0=$link->prepare($sql0); $stmt0->execute();
	
	if(!isset($_POST['btnCalcSTL'])){
		$sql='SELECT * FROM CalcIncentiveTable;';
		include_once('../backendphp/layout/displayastablenosort.php');
	}
	
	if(isset($_POST['btnLookupSTLIncentives'])){
		header('Location:incentives.php?w=CalcIncentivesSTL&MonthNo=2'.$quarter);
		exit();
	}
	
	
	if(isset($_POST['btnCalcSTL'])){
		$sqltotal='SELECT COUNT(sfb.BranchNo) AS TotHandledBranch,SUM(IF(Target'.$quarter.'QReached="Yes",1,0)) AS NotReached, SUM(Total_Incentive'.$quarter.'Q) AS Incentive'.$quarter.'Q,CONCAT(e.Nickname," ",e.SurName) AS STL,sfb.IDNo FROM CalcIncentiveTable cit JOIN STLFixedBranch sfb ON cit.BranchNo=sfb.BranchNo LEFT JOIN 1employees e ON sfb.IDNo=e.IDNo GROUP BY sfb.IDNo;';
		$stmttotal=$link->query($sqltotal); $resulttotal=$stmttotal->fetchAll();
		
		foreach($resulttotal AS $resulttot){
			if($resulttot['TotHandledBranch']==$resulttot['NotReached']){
				$sqlinsert='INSERT INTO hr_2incentivesub SET MonthNo="2'.$quarter.'",BranchNo='.$resulttot['IDNo'].',IDNo='.$resulttot['IDNo'].',PositionID=36,Amount="'.$resulttot['Incentive'.$quarter.'Q'].'",EncodedByNo='.$_SESSION['(ak0)'].',TimeStamp=NOW();';
...
/* case 'CalcIncentivesSTL':
if (!allowedToOpen(array(2014,2015,2016),'1rtc')) { echo 'No permission'; exit;}
$txndate=$_GET['MonthNo'];
$title='STL Incentives (Quarter: '.substr($txndate,-1).')';
$sqlc='SELECT COUNT(TxnSubId) as CntTotal FROM hr_2incentivesub WHERE MonthNo='.$txndate.'';
	$stmtc=$link->query($sqlc); $resultc=$stmtc->fetch();
if($resultc['CntTotal']==0){
...
$sqlb='SELECT Posted,Approved,Sent FROM hr_1incentivemain WHERE MonthNo='.$txndate.'';
	...
	if(allowedToOpen(2014,'1rtc')){
		if($resultb['Posted']==0 AND $resultb['Approved']==0 AND $resultb['Sent']==0){
			echo '<div style="width:30%;"><div style="float:left;"><form action="incentives.php?w=PostUnpost&MonthNo='.$txndate.'" method="POST"><input style="background-color:yellow;color:#000000;padding:2px;" type="submit" name="btnPost" value="I have verified the values. Post" OnClick="return confirm(\'Verified?\');"></form></div><div style="float:right">';
	...
	if(allowedToOpen(2015,'1rtc')){
		if($resultb['Posted']==1 AND $resultb['Approved']==0 AND $resultb['Sent']==0){
			echo '<div style="width:30%;"><div style="float:left;"><form action="incentives.php?w=Approve&MonthNo='.$txndate.'" method="POST"><input style="background-color:blue;color:#ffffff;padding:2px;" type="submit" name="btnPost" value="Approve Incentives" OnClick="return confirm(\'Approved?\');"></form></div><div style="float:right">';
	...
	if(allowedToOpen(2016,'1rtc')){
		if($resultb['Posted']==1 AND $resultb['Approved']==1 AND $resultb['Sent']==0){
			echo '<div style="width:30%;"><div style="float:left;"><form action="incentives.php?w=SendToSpecialCredits&MonthNo='.$txndate.'" method="POST">Batch: <input type="text" value="01" name="Batch" size="10"> <input style="background-color:green;color:#ffffff;padding:2px;" type="submit" name="btnSend" value="Send To Special Credits" OnClick="return confirm(\'Send To Special Credits?\');"></form></div><div style="float:right">';
...
$sql='SELECT `is`.IDNo,Amount AS Incentives,CONCAT(e.Nickname," ",e.SurName) AS STL from hr_1incentivemain im JOIN hr_2incentivesub `is` ON im.MonthNo=`is`.MonthNo JOIN 1employees e ON e.IDNo=`is`.IDNo WHERE im.MonthNo='.$txndate.';';
$columnnames=array('IDNo','STL','Incentives');
include_once('../backendphp/layout/displayastablenosort.php');
break; */
