...
case 'PerDept':
  ...
  JOIN (SELECT JobLevelNo, jc.JobClassNo, FORMAT(MinRate,0) AS `MINIMUM`, FORMAT(MinRate*(1+PercentMintoMed/100),-1) AS MEDIAN, FORMAT(MinRate*(1+PercentMintoMed/100)*(1+PercentMedtoMax/100),-1) AS MAXIMUM  FROM attend_1joblevel jl JOIN attend_0jobclass jc ON jc.JobClassNo=jl.JobClassNo GROUP BY jl.JobLevelNo ORDER BY JobClassNo,JobLevelNo) jl ON jl.JobLevelNo=p.JobLevelNo
...
case 'Approve':
  ...
  JOIN attend_0positions p ON p.PositionID=m.PositionID
