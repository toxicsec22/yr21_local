...
AND IDNo NOT IN (SELECT IDNo FROM '.$nextyr.'_1rtc.payroll_2requestoicallowance);';
    ...
    case 'attend_2changeofpositions':
     ...
     case '1employeesSIL':
         ...
         $sql0='UPDATE `' . $nextyr . '_1rtc`.`1employees` e LEFT JOIN `' . $currentyr . '_1rtc`.`attend_61silbal` a ON e.IDNo=a.IDNo JOIN `' . $nextyr . '_1rtc`.`attend_30currentpositions` cp ON e.IDNo=cp.IDNo JOIN `' . $nextyr . '_1rtc`.`attend_1positions` p ON p.PositionID=cp.PositionID JOIN `' . $currentyr . '_1rtc`.`attend_howlongwithus` h ON e.IDNo=h.IDNo 
