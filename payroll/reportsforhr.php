...
?>
	...
	switch ($which){
		...
		$title='Salaries per Job Level';
			$sql1='SELECT JobLevelID, CONCAT(JobClassification," Level ", RIGHT(JobLevelID,1)) AS JobLevel FROM `attend_0joblevels` jl JOIN `attend_0jobclass` jc ON jc.JobClassNo=jl.JobClassNo '.(!allowedToOpen(8041,'1rtc')?' WHERE jc.JobClassNo NOT IN (7)':'').' ORDER BY jc.JobClassNo, jl.JobLevelID ';
