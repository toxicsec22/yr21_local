...
foreach ($months as $fsmonth){
   ...
   $sql0is=$sql0is.', FORMAT(IFNULL(SUM(`'.$monthcol.'`)*ca.NormBal,0)*IF(ca.ContraAccountOf<>0,(-1),1),2) as `'.$monthname.'`';
   $sql0bs=$sql0bs.', FORMAT(IFNULL(SUM(`'.$monthcol.'asof`)*ca.NormBal,0)*IF(ca.ContraAccountOf<>0,(-1),1),2) as `'.$monthname.'`';
...
//   echo $sql0is.'<br>'.'<br>';
// echo $sql0bs.'<br>'.$reportmonth.'<br>'.$closedmonth; exit();
...
// print_r ($months); break;
   ...
   foreach ($resultacct as $acctid){
      ...
      $stmt=$link->prepare($sqlupdatebs);$stmt->execute();
      /*$sqlupdatebs='UPDATE `'.$currentyr.'_static`.`acctg_bsvalues` fs JOIN `acctg_1begbal` beg on beg.AccountID=fs.AccountID JOIN acctg_1chartofaccounts ca on ca.AccountID=fs.AccountID SET `BegValue`=beg.BegBalance, `Beginning`=(beg.BegBalance*ca.NormBal*IF(ContraAccountOf<>0,(-1),1)) where fs.AccountID='.$acctid['AccountID'];
      $stmt=$link->prepare($sqlupdatebs);$stmt->execute();
      $stmt=$link->prepare($sqlupdateis);$stmt->execute();*/
   }
