...
UNION ALL
SELECT "Sale", IF(ss.ClientNo<9999,"E","C") AS `BECS`, ss.ClientNo as `SuppNo/ClientNo`, ss.CreditAccountID, sm.BranchNo,sm.BranchNo as FromBudgetOf, Sum(Amount)*-1 AS Expr2, "CR" AS Expr3
FROM acctg_2salemain sm INNER JOIN acctg_2salesub ss ON (sm.TxnID = ss.TxnID)
...
// COLLECTIONS
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "Collect", IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo as `SuppNo/ClientNo`, orm.DebitAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, Sum(ors.Amount), "DR" AS Expr3
...
UNION ALL
SELECT "Collect", IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo as `SuppNo/ClientNo`, orm.DebitAccountID as CreditAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, Sum(ors.Amount)*-1, "CR" AS Expr3
...
UNION ALL SELECT "Collect", IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ors.CreditAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, Sum(ors.Amount)*-1 AS Expr3, "CR" AS Expr4
FROM acctg_2collectmain orm  INNER JOIN acctg_2collectsub ors ON  (orm.TxnID = ors.TxnID)
...
UNION ALL SELECT "Collect", IF(orm.ClientNo<9999,"E","C") AS `BECS`, orm.ClientNo, ors.DebitAccountID, ors.BranchNo,ors.BranchNo as FromBudgetOf, Sum(ors.Amount) AS Expr3, "DR" AS Expr4
FROM acctg_2collectmain orm  INNER JOIN acctg_2collectsubdeduct ors ON  (orm.TxnID = ors.TxnID)
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "Dep#", IF(deps.ClientNo<9999,"E","C") AS `BECS`, deps.ClientNo  as `SuppNo/ClientNo`, depm.DebitAccountID, deps.BranchNo,deps.BranchNo as FromBudgetOf, Sum(deps.Amount), "DR" AS Expr4
...
UNION ALL
SELECT "Encash", "B" AS `BECS`, depe.BranchNo, depe.DebitAccountID, depe.BranchNo,FromBudgetOf, Sum(depe.Amount), "DR" AS Expr4
...
UNION ALL
SELECT "Dep", IF(deps.ClientNo<9999,"E","C") AS `BECS`, deps.ClientNo as `SuppNo/ClientNo`, deps.CreditAccountID, deps.BranchNo,deps.BranchNo as FromBudgetOf, Sum(deps.Amount)*-1 AS Expr4, "CR" AS Expr5
FROM acctg_2depositmain depm INNER JOIN acctg_2depositsub deps ON depm.TxnID = deps.TxnID
...
UNION ALL
SELECT "Encash", "B" AS `BECS`, depe.BranchNo as `SuppNo/ClientNo`, depm.DebitAccountID AS CreditAcctID, depe.BranchNo,FromBudgetOf, Sum(`Amount`)*-1 AS Expr3, "CR" AS Expr4
FROM acctg_2depositmain depm INNER JOIN acctg_2depencashsub depe ON depm.TxnID = depe.TxnID
...
$sqlalltxns=$sqlalltxns.' UNION ALL SELECT "BouncedCollectLastYr", IF(bm.ClientNo<9999,"E","C") AS `BECS`, bm.ClientNo, 200 AS DebitAccountID, BranchNo,BranchNo as FromBudgetOf, Sum(AmountofPDC), "DR"
...
WHERE Year(DateBounced)='.$thisyr.' AND MONTH(DateBounced) in ('.$lastmonth.') and 200 in '.$acctid.'
    GROUP BY BranchNo,bm.ClientNo
     UNION ALL SELECT "BouncedCollectLastYr",IF(bm.ClientNo<9999,"E","C") AS `BECS`, bm.ClientNo, IFNULL(CreditAccountID,100) AS CreditAccountID, BranchNo,BranchNo as FromBudgetOf, Sum(AmountofPDC)*-1, "CR"
FROM `acctg_3undepositedpdcfromlastperiod` bm JOIN `acctg_3undepositedpdcfromlastperiodbounced` bs ON  (bm.UndepPDCId = bs.UndepPDCId)
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "Bounced", IF(cm.ClientNo<9999,"E","C") AS `BECS`, cm.ClientNo, 200 AS DebitAccountID, cs.BranchNo,cs.BranchNo as FromBudgetOf, Sum(cs.Amount), "DR" AS Expr4
...
UNION ALL
SELECT "Bounced", IF(cm.ClientNo<9999,"E","C") AS `BECS`, cm.ClientNo, IFNULL(cbs.CreditAccountID,100) AS CreditAccountID, cs.BranchNo,cs.BranchNo as FromBudgetOf, Sum(`Amount`)*-1 AS Expr3, "CR" AS Expr4
FROM `acctg_2collectmain` `cm` JOIN  `acctg_2collectsub` `cs` ON `cm`.`TxnID` = `cs`.`TxnID` JOIN `acctg_2collectsubbounced` `cbs` ON `cm`.`TxnID` = `cbs`.`TxnID`
...
UNION ALL
SELECT "Bounced", IF(cm.ClientNo<9999,"E","C") AS `BECS`, cm.ClientNo, IFNULL(cbb.CreditAccountID,100) AS DebitAccountID, cbs.BranchNo,cbs.BranchNo as FromBudgetOf, SUM(cbs.Amount), "DR" AS Expr3
...
UNION ALL
SELECT "Bounced", IF(cm.ClientNo<9999,"E","C") AS `BECS`, cm.ClientNo, IFNULL(cbs.DebitAccountID,100) as CreditAccountID, cbs.BranchNo,cbs.BranchNo as FromBudgetOf, SUM(cbs.Amount)*-1, "CR" AS Expr3
FROM acctg_2collectmain cm JOIN `acctg_2collectsubdeduct` cbs ON  (cm.TxnID = cbs.TxnID)   JOIN `acctg_2collectsubbounced` cbb ON  (cm.TxnID = cbb.TxnID) 
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrOUT", "B" AS `BECS`, tm.FromBranchNo,ts.DebitAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, Sum(ts.Amount), "DR" AS Expr2
...
UNION ALL
SELECT "TxfrOUT", "B" AS `BECS`, tm.FromBranchNo,tm.CreditAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, Sum(ts.Amount)*-1, "CR" AS Expr2
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
if (in_array(300,$acctidarray)){
        ...
        $sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrIN" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo, 300 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, Sum(ts.Amount), "DR" AS Expr3 FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
if (in_array(330,$acctidarray)){ //In Transit account 
        ...
        $sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrIN" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo, 330 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, Sum(ts.Amount), "DR" AS Expr3 FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
if (in_array(404,$acctidarray)){ //APTradeTxfr account
        // DateIN Is Not Null
        $sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrIN" AS Expr1, "B" AS `BECS`, ts.ClientBranchNo,404 AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, Sum(ts.Amount)*-1, "CR" AS Expr3 FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrOUTRecvPayment", "B" AS `BECS`, tm.FromBranchNo,ts.PaidViaAcctID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, Sum(ts.Amount), "DR" AS Expr2
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
UNION ALL
SELECT "TxfrOUTRecvPayment", "B" AS `BECS`, tm.FromBranchNo,ts.DebitAccountID, tm.FromBranchNo,tm.FromBranchNo as FromBudgetOf, Sum(ts.Amount)*-1, "CR" AS Expr2
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrINPayment", "B" AS `BECS`, ts.ClientBranchNo,ts.PaidViaAcctID AS CRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, Sum(ts.Amount)*-1, "CR" AS Expr3
FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
...
$sqllastmonth=$sqllastmonth.' UNION ALL SELECT "TxfrINPayment" AS Expr1,"B" AS `BECS`, ts.ClientBranchNo,404 AS DRID, ts.ClientBranchNo,ts.ClientBranchNo as FromBudgetOf, Sum(ts.Amount), "DR" AS Expr3 FROM acctg_2txfrmain tm INNER JOIN acctg_2txfrsub ts ON tm.TxnID = ts.TxnID
