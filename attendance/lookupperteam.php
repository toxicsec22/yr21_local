...
$deptcondition='(((IF('.$_SESSION['&pos'].' IN (99,100),supervisorpositionid,IF( '.$_SESSION['&pos'].' IN (SELECT deptheadpositionid FROM `1departments`), deptheadpositionid, supervisorpositionid)))='.$_SESSION['&pos'].' OR p.LatestSupervisorIDNo='.$_SESSION['(ak0)'].') '.((allowedToOpen(618,'1rtc'))?' OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE OpsSpecialist='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE SAM='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE TeamLeader='.$_SESSION['(ak0)'].')) OR (p.IDNo IN (SELECT IDNo FROM attend_30currentpositions cp JOIN attend_1branchgroups bg ON cp.BranchNo=bg.BranchNo WHERE CNC='.$_SESSION['(ak0)'].'))':'').')';
   ...
   case 'SLDiscrepancies':
	...
	$link=connect_db(''.$currentyr.'_1rtc',0);
       ...
       if(allowedToOpen(612, '1rtc')){
           ...
           $sql='SELECT b.Branch, e.IDNo, concat(FirstName,\' \',SurName,IF(Resigned<>0," - RESIGNED","")) as `FullName`, Count(DateToday) AS CountOfDate FROM `1branches` as b INNER JOIN (1employees e INNER JOIN `attend_2attendance` as a ON e.IDNo = a.IDNo) ON b.BranchNo = a.BranchNo WHERE '.$deptcondition.' AND ((TimeIn) Is Not Null) and DateToday>=\''. $_POST['FromDate']  . '\' and DateToday<=\''. $_POST['ToDate']  . '\'  GROUP BY e.IDNo,a.BranchNo ORDER BY Branch, FullName'; //JOIN attend_30currentpositions p ON e.IDNo=p.IDNo  JOIN attend_30currentpositions p ON e.IDNo=p.IDNo 
