...
$columnnames=array('Date','SerialNo','Branch','Refrigerant','Vacuum','VacuumedBy','AuditedBy','TotalSoldPerTank','TotalAccounted','Posted','PostedByNo');
    ...
    foreach($result as $row){
        ...
        $sub=$sub.(editOk('audit_3vacuum',$row[$txnidname],$link,'vacuum')?'<td><a href="prvacuum.php?w=TotalSold&action_token='.$_SESSION['action_token'].'&CountID='.$row[$txnidname].'">Update Total Sold</a>'.str_repeat('&nbsp',8).'</td><td><a href=../backendphp/layout/postunpost.php?Table=audit_3vacuum&Link=audit&txntype=vacuum&Post=1&TxnID='.$row[$txnidname].'>Post</a>'.str_repeat('&nbsp',8).'<a href="editauditspecifics.php?edit=2&w=VacuumEdit&CountID='.$row[$txnidname].'">Edit</a>'.str_repeat('&nbsp',8).'<a href=prvacuum.php?CountID='.$row[$txnidname].'&action_token='.$_SESSION['action_token'].'&w=VacuumDel OnClick="return confirm(\'Really delete this?\');">Delete</a></td>':'<td><a href=../backendphp/layout/postunpost.php?Table=audit_3vacuum&Link=audit&txntype=vacuum&Post=0&TxnID='.$row[$txnidname].'>Unpost</a></td>').'</tr>';
