...
$sql0='CREATE TEMPORARY TABLE ItemAct AS SELECT `Date`, `From`, `MRRNo` AS Number, `BECS`, `SupplierNo` AS BECSNo,`ItemCode`,`Qty`,`UnitCost`,`UnitPrice`,`SerialNo`,`BranchNo`,`ActRemarks`,`txntype`,`Defective`
    ...
    UNION ALL SELECT `Date`,"Sale" AS `From`, `SaleNo`, IF(ClientNo<9999,"E","C") AS `BECS`, `ClientNo`, `ItemCode`, (`ss`.`Qty` * -(1)) AS `Qty`,
