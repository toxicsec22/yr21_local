...
if (in_array($which,array('AssignUtilityBill','ReceiveBill','EditSpecificsAssignUtilityBill','MonitorBills','SendToCV','AssignmentHistory'))){
   ...
   $sql1='SELECT ua.*,IF(ua.Active=1,"Yes","No") AS `Active?`,'.$fieldastxn.'BillType,ShortAcctID,SupplierName AS BillerName,b.Branch AS ExpenseOf, IF(AssigneeNo<1000,b2.Branch,CONCAT(id.Nickname," ",id.Surname)) AS Assignee,CONCAT(id3.Nickname," ",id3.Surname) AS EncodedBy,Company AS DeclaredforCompany FROM acctg_4billassignment ua LEFT JOIN acctg_0billtype bt ON ua.BTID=bt.BTID LEFT JOIN 1suppliers s ON ua.BillerID=s.SupplierNo LEFT JOIN acctg_1chartofaccounts coa ON ua.ExpenseAccountID=coa.AccountID LEFT JOIN 1_gamit.0idinfo id ON ua.AssigneeNo=id.IDNo LEFT JOIN 1companies c ON ua.DeclaredforCompanyNo=c.CompanyNo LEFT JOIN 1branches b ON ua.ExpenseOfBranchNo=b.BranchNo LEFT JOIN 1branches b2 ON ua.AssigneeNo=b2.BranchNo LEFT JOIN 1_gamit.0idinfo id3 ON ua.EncodedByNo=id3.IDNo '; //echo $sql1;
   
   $columnnameslist=array('BillerName','BillType','SubscriberName', 'AccountNo','ShortAcctID', 'TelMobileNo', 'MRF','AddOns','PlanDescription', 'Assignee', 'ExpenseOf', 'DeclaredforCompany','CutOffDay','DueDay','DateAssigned','EndOfContract','Remarks','Active?','Timestamp','EncodedBy');
